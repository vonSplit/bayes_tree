...
TEST_CASE("Check normalised probs") {
...
TEST_CASE("Log-likelihood test with known result") {
    std::vector<double> probs = {0.2, 0.3, 0.5};
    CategoricalDistribution dist(probs);
...
TEST_CASE("Edge case: zero count shouldn't affect log-likelihood") {
    std::vector<double> probs = {0.2, 0.3, 0.5};
    CategoricalDistribution dist(probs);
...
TEST_CASE("Impossible event: zero-probability category with nonzero count") {
